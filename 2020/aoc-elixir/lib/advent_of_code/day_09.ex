defmodule AdventOfCode.Day09 do
  ...
  def find_bad_encoding(nums) do
    ...
    |> Stream.reject(&(List.last(&1) in (for {x, i} <- Enum.with_index(Enum.take(&1, 25)),
                                             y <- Enum.drop(&1, i) |> Enum.take(25), do: x + y)))
    |> Enum.take(1) |> List.first |> List.last
  end
  ...
  def part2(args) do
    ...
    |> Enum.take(1) |> List.first |> (&(Enum.min(&1) + Enum.max(&1))).()
  end
end
